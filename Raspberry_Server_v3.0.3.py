...
def system_status():
    """Get detailed system status and information"""
    if request.method == 'OPTIONS':
        return make_response('', 200)
    ...
    try:
        ...
        status = {
            ...
            "active_timers": len(active_timers),
            "relay_states": {pin: RELAY_PINS[pin]["state"] for pin in RELAY_PINS},
            "timestamp": datetime.now().isoformat(),
...
@app.route('/system/ping', methods=['GET', 'OPTIONS'])
def system_ping():
    """Simple ping endpoint with system info"""
    if request.method == 'OPTIONS':
        return make_response('', 200)
...
@app.route('/system/reboot', methods=['GET', 'OPTIONS'])
def reboot_pi():
    """Reboot the Raspberry Pi"""
    if request.method == 'OPTIONS':
        return make_response('', 200)
    ...
    try:
        ...
        response = jsonify({
            "status": "success",
            "message": "Raspberry Pi rebooting in 5 seconds...",
            "timestamp": datetime.now().isoformat()
        })
...
@app.route('/system/shutdown', methods=['GET', 'OPTIONS'])
def shutdown_pi():
    """Gracefully shutdown the Raspberry Pi"""
    if request.method == 'OPTIONS':
        return make_response('', 200)
    ...
    try:
        ...
        response = jsonify({
            "status": "success",
            "message": "Raspberry Pi shutting down in 5 seconds...",
            "timestamp": datetime.now().isoformat()
        })
...
@app.route('/system/service/restart', methods=['GET', 'OPTIONS'])
def restart_service():
    """Restart the relay server service"""
    if request.method == 'OPTIONS':
        return make_response('', 200)
    
    try:
        logger.info("Service restart command received from %s", request.remote_addr)
        
        response = jsonify({
            "status": "success",
            "message": "Relay server service will restart in 3 seconds...",
            "timestamp": datetime.now().isoformat()
        })
        
        def delayed_restart():
            time.sleep(3)
            logger.info("Restarting relay server service...")
            # This will restart the current process
            os.system("sudo systemctl restart darkroom-relay.service")
